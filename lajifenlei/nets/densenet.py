...
model_urls = {
    'densenet_121': 'https://download.pytorch.org/models/densenet121-a639ec97.pth',
}
...
def densenet_121(pretrained=False, progress=True, num_classes=1000):
    model = densenet121()
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls['densenet_121'], model_dir='./model_data',
                                              progress=progress)
         # 但是最新的torch不允许出现"."，所以老版权重恢复进新版模型时需要修改一下模块命名
        pattern = re.compile(
            r"^(.*denselayer\d+\.(?:norm|relu|conv))\.((?:[12])\.(?:weight|bias|running_mean|running_var))$")
        for key in list(state_dict.keys()):  # 主要是新版模型中的最基础模块的命名是类似于...denselayer1.conv1.weight
