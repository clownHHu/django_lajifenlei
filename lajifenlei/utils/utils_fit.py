...
def fit_one_epoch(model_train, model, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch,
    ...
    if epoch + 1 == Epoch and model_name != '':
        if not os.path.exists('logs/'+model_name):
            os.makedirs('logs/'+model_name)
        torch.save(model.state_dict(), 'logs/'+model_name+'/ep%03d-loss%.3f-val_loss%.3f.pth' % (
            (epoch + 1), total_loss / epoch_step, val_loss / epoch_step_val))
        # upload(model_name, 'logs/ep%03d-loss%.3f-val_loss%.3f.pth' % (
        #     (epoch + 1), total_loss / epoch_step, val_loss / epoch_step_val))
